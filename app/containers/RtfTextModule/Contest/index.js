...
if(this.state.sortedSelectedRows.status==="LIVE"){
 ...
 if(!thmId){
    ...
    var rwdVals=rwdTypes?rwdVal?rwdVal:"":"";
 ...
 if(res.data.ottresp.sts==1){
  ...
  addContest=()=>{
   ...
   CALL_API("post",API_URL+"ottaddcontest.json?clId=AMIT202002CID&name="+encodeURIComponent(name)+"&stDate="+moment(new Date(stDate)).format("MM/DD/YYYY")+"&enDate="+moment(new Date(enDate)).format("MM/DD/YYYY")+"&cat="+cat+"&subCat="+subCats+"&qMode="+qMode+"&qType="+qType+"&elmType="+elmType+"&rwdType="+rwdTyp+"&rwdVal="+rwdVals+"&qSize="+qSize+"&dSeq="+dSeq+"&thmId="+thmId+"&themeColor="+themeColor+"&cau="+udata.userid).then((res)=>{
     //console.log(res);
     var result=res.data.ottresp;
     if(res.data.ottresp.sts===0){
          message.error(res.data.ottresp.msg);
...
slectTheme=(thmId)=>{
  ...
  getContest=(loadingd)=>{
    ...
    CALL_API("post",API_URL+"ottgetallcontests.json?clId=AMIT202002CID&sts=ACTIVE&pgNo="+this.state.pgNo+"&cau="+udata.userid+"&hfilter="+fil).then((res)=>{
  //console.log(res);
  if(res.data.ottresp.sts===0){
          message.error(res.data.ottresp.msg);
     }
...
{name:"Action",key:"Action",formatter: buttonFormater,width:160},
  ...
  return (
    ...
    <Button onClick={this.startContest}  variant="contained"  color="primary" className={classes.button}>
              START CONTEST 
        </Button> 
