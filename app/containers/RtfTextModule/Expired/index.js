...
if(this.state.sortedSelectedRows.status==="LIVE"){
      message.error("Contest is LIVE, can not add prize")
  return;
}
}
...
deleteContest= async (id)=>{
 ...
 var d=datas.filter((e,i)=>{
   ...
   const {coId}=d;
CALL_API("get",API_URL+"ottdeletecontest.json?clId=AMIT202002CID&coId="+coId+"&cau="+udata.userid).then((res)=>{
      //console.log(res);
...
console.log(this.state.sortedSelectedRows);
...
if(this.state.sortedSelectedRows.status==="LIVE"){
  ...
  return;
   ...
   const {name,stDate,coId,enDate,cat,subCat,qMode,ansType, elmType,rwdType,rwdVal,qSize,seqNo,thmId,themeColor,isElimination}=d;
   //console.log(d);
   this.getSubCat(cat);
    this.setState({
        name,stDate:moment(stDate).format("MM/DD/YYYY"),enDate:moment(enDate).format("MM/DD/YYYY"),coId,cat,subCat,qMode,qType:ansType,elmType:isElimination==false?"false":"true",rwdType,rwdVal,qSize,dSeq:seqNo,thmId,themeColor:"000",
        isEdit:true
    });
    this.handleOpen();
  }
  ...
  const {name,stDate,enDate,cat,subCat,qMode,coId,qType,elmType,rwdType,rwdVal,qSize,dSeq,thmId,themeColor,sortedSelectedRows}=this.state;


    if(!name || !stDate || !enDate || !cat  || !qMode || !qType   || !qSize  || String(dSeq)=="" || typeof dSeq=="undefined"){
 ...
 if(!thmId){
    ...
    if(!rwdVal){
     message.error("Prize Quantity is mandatory")
      this.setState({
        showError:true
      });
      return;
    }
    var rwdTypes=rwdType?rwdType:"";
    var rwdVals=rwdVal?rwdVal:"";
    var rwdTyp=rwdType?rwdType:"";




   CALL_API("post",API_URL+"ottupdatecontest.json?clId=AMIT202002CID&coId="+coId+"&name="+encodeURIComponent(name)+"&stDate="+stDate+"&enDate="+enDate+"&cat="+cat+"&subCat="+subCat+"&qMode="+qMode+"&qType="+qType+"&elmType="+elmType+"&rwdType="+rwdTypes+"&rwdVal="+rwdVals+"&qSize="+qSize+"&dSeq="+dSeq+"&thmId="+thmId+"&themeColor="+themeColor+"&cau="+udata.userid).then((res)=>{
 ...
 if(res.data.ottresp.sts==1){
  ...
  addContest=()=>{
    const {name,stDate,enDate,cat,subCat,qMode,qType,elmType,rwdType,rwdVal,qSize,dSeq,thmId,themeColor}=this.state;
    ...
    if(!name || !stDate || !enDate || !cat  || !qMode || !qType   || !qSize  || String(dSeq)=="" || typeof dSeq=="undefined"){
      ...
      // console.log(name,stDate,enDate,cat,qMode,qType,qSize,dSeq);
    ...
    if(!subCat){
     message.error("Sub category is mandatory")
      this.setState({
        showError:true
      });
      return;
    ...
    if(new Date(stDate) > new Date(enDate)){
      message.error("Start Date must be Smaller then   End Date!")
      this.setState({
        showError:true
      });
      return; 
   ...
   CALL_API("post",API_URL+"ottaddcontest.json?clId=AMIT202002CID&name="+encodeURIComponent(name)+"&stDate="+moment(new Date(stDate)).format("MM/DD/YYYY")+"&enDate="+moment(new Date(enDate)).format("MM/DD/YYYY")+"&cat="+cat+"&subCat="+subCat+"&qMode="+qMode+"&qType="+qType+"&elmType="+elmType+"&rwdType="+rwdTyp+"&rwdVal="+rwdVals+"&qSize="+qSize+"&dSeq="+dSeq+"&thmId="+thmId+"&themeColor="+themeColor+"&cau="+udata.userid).then((res)=>{
     //console.log(res);
     var result=res.data.ottresp;
     if(res.data.ottresp.sts===0){
          message.error(res.data.ottresp.msg);
...
slectTheme=(thmId)=>{
  ...
  getContest=(loadingd)=>{
    ...
    CALL_API("post",API_URL+"ottgetallcontests.json?clId=AMIT202002CID&sts=EXPIRED&pgNo="+this.state.pgNo+"&cau="+udata.userid+"&hfilter="+fil).then((res)=>{
  //console.log(res);
  if(res.data.ottresp.sts===0){
          message.error(res.data.ottresp.msg);
     }
   datas=res.data.ottresp.contests.map((e)=>{
     e.stDate=moment(new Date(e.stDate)).format("MM/DD/YYYY");
     e.enDate=moment(new Date(e.enDate)).format("MM/DD/YYYY");
      e.crDate=moment(new Date(e.crDate)).format('DD/MM/YYYY')
     e.upDate=moment(new Date(e.upDate)).format('DD/MM/YYYY')
     return e;
   })
...
if(loadingd){
  loadingd.remove();
          ...
          message.success("Something went Worng");
});
...
CALL_API("post",API_URL+"ottstartcontest.json?clId=AMIT202002CID&coId="+this.state.coId).then((res)=>{
  // console.log(e);
  if(res.data.ottresp.sts===0){
            message.error(res.data.ottresp.msg);
    }else{
...
CALL_API("post",API_URL+"ottendcontest.json?clId=AMIT202002CID&coId="+this.state.coId).then((res)=>{
  // console.log(e);
  if(res.data.ottresp.sts===0){
          message.error(res.data.ottresp.msg);    
  }else{
...
{name:"Elimination",key:"isElimination",formatter: NoFalseformatter},
{name:"Winner Prize",key:"rwdType",formatter: formatter},
{name:"Prize Quantity",key:"rwdVal",formatter: formatter},
{name:"No of Questions",key:"qSize",formatter: formatter},
        ...
        />
          ...
          helperText={
            !this.state.enDate && showError?"This field is required":"" || 
              new Date(this.state.stDate)> new Date(this.state.enDate)?" End Date should be greater than Start Date":""
          }
...
<div style={{paddingLeft:10,paddingRight:10,flex:1}}>
          <FormControl style={{width:'100%'}}  className={classes.formControl}>
            ...
            <FormControl style={{width:'100%'}}  className={classes.formControl}>
            <InputLabel htmlFor="age-simple">Winner Prize </InputLabel>
            <Select
              value={this.state.rwdType}
              onChange={this.handleChange}
              displayEmpty="true"
              helperText="this field is required"
              inputProps={{
            ...
            >
              ...
              {this.state.rewardList && this.state.rewardList.map((e)=>{
            ...
            <div style={{paddingLeft:10,paddingRight:10,flex:1}}>
              <FormControl style={{width:'100%'}} className={classes.formControl}>
                <InputLabel htmlFor="name-simple"> Prize Quantity</InputLabel>
                <Input 
                disabled={!this.state.rwdType}
                  type="tel"
                id="name-simple" name="rwdVal" value={this.state.rwdVal} onChange={this.handleChange} />
                {this.state.rwdType &&  !this.state.rwdVal && showError && <span className="err">{LABLES.required}</span>}
...
<div className="flex_div">
            <div style={{paddingLeft:10,paddingRight:10,flex:1}}>
              <FormControl style={{width:'100%'}} className={classes.formControl}>
                <InputLabel htmlFor="name-simple"> No of Questions *</InputLabel>
                <Input
               error={!this.state.qSize && showError}
          helperText="this field is required"
          type="tel"
           id="name-simple" name="qSize" value={this.state.qSize} onChange={this.handleChange} />
                {!this.state.qSize && showError && <span className="err">{LABLES.required}</span>}
