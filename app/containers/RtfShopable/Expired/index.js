...
CALL_API("get",API_URL+"snwdeletecontest.json?clId=AMIT202002CID&coId="+coId).then((res)=>{
  ...
  editContent=(id)=>{
   ...
   const {name,stDate,coId,enDate,cat,subCat,qMode,ansType, elmType,rwdType,rwdVal,qSize,seqNo,thmId,themeColor,playInterval}=d;
   //console.log(d);
   this.getSubCat(cat);
    this.setState({
...
if(!name || !stDate || !enDate || !cat  || !qMode || !qType     || !dSeq || !playInterval){
 ...
 if(!thmId){
   ...
   CALL_API("post",API_URL+"snwupdatecontest.json?clId=AMIT202002CID&coId="+coId+"&name="+name+"&stDate="+stDate+"&enDate="+enDate+"&cat="+cat+"&subCat="+subCat+"&qMode="+qMode+"&qType="+qType+"&qSize="+qSize+"&dSeq="+dSeq+"&thmId="+thmId+"&themeColor="+themeColor+"&playInterval="+playInterval).then((res)=>{
        ...
        />
          ...
          helperText={
            !this.state.enDate && showError?"This field is required":"" || 
              new Date(this.state.stDate)> new Date(this.state.enDate)?" start date to be greater that end date":""
          }
          disablePast={!this.state.isEdit}
          variant="inline"
          format="MM/dd/yyyy"
